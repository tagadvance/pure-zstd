...
TILE = pathlib.Path("path/to/srtm1z/N00E042.hgtz")
TILE_BLOCK = 183
...
def build():
    ...
    #     .hgtz corpus has 249 of these and the committed container has none.
    tile = TILE.read_bytes()
    offset, length = struct.unpack_from(">II", tile, 12 + 8 * TILE_BLOCK)
    frame = bytes(tile[offset : offset + length])
    assert direct_nibble_weights(frame), "that block does not use direct nibbles"
    expect = plain("direct_nibble", pyzstd.decompress(frame))
    legal(
        "direct_nibble",
        frame,
        [expect],
        f"block {TILE_BLOCK} of {TILE}, lifted out as its own frame; its "
        "literals carry the direct-nibble Huffman weight form",
    )
